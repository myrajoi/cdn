...
Use the facade 'Cdn' to call the `Cdn::mix()` function.
*Note: the `mix` works the same as the Laravel `mix` it loads the mix-manifest.json file from build folder and chooses the correct file revision generated by webpack:*
```blade
{{Cdn::mix('assets/js/main.js')}}        // example result: https://js-bucket.s3.amazonaws.com/public/build/assets/js/main-85cafe36ff.js

{{Cdn::mix('assets/css/style.css')}}        // example result: https://css-bucket.s3.amazonaws.com/public/build/assets/css/style-2d558139f2.css
```
